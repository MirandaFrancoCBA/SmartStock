...
class Command(BaseCommand):
    help = "Create SmartStock RBAC roles and optionally assign an existing user."

    def add_arguments(self, parser):
        parser.add_argument("--username", help="Existing username to assign to a role.")
        parser.add_argument(
            "--role",
            choices=ROLE_NAMES,
            help="SmartStock role to assign. Requires --username.",
        )
    ...
    def handle(self, *args, **options):
        ...
        if not username:
            self.stdout.write(self.style.SUCCESS("SmartStock RBAC roles are ready."))
            return
